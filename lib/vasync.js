...
function forEachPipeline(args, callback) {
    ...
    mod_assert.equal(typeof (args['func']), 'function',
		'"args.func" must be specified and must be a function');
    mod_assert.ok(Array.isArray(args['inputs']) || typeof args['inputs'] === 'object',
		'"args.inputs" must be specified and must be an array or object');
    mod_assert.equal(typeof (callback), 'function',
		'callback argument must be specified and must be a function');
    ...
    if (Array.isArray(args['inputs'])) {
        ...
        for (var key in args['inputs']) {
            if (args['inputs'].hasOwnProperty(key)) {
                funcs.push(function (_, subcallback) {
                  return (func(args['inputs'][key], subcallback));
                });
            }
        }
    }
